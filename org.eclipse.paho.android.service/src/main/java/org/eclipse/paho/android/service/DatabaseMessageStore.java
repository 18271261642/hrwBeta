...
//		c.close();
	...
	public boolean discardArrived(String clientHandle, String id) {
		...
		try {
			String sql = "delete from "+ARRIVED_MESSAGE_TABLE_NAME+" where "+
					MqttServiceConstants.MESSAGE_ID + "="+selectionArgs[0]+" AND "
					+ MqttServiceConstants.CLIENT_HANDLE + "="+selectionArgs[1];
			 db.execSQL(sql);
