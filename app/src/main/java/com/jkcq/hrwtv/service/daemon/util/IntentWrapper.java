package com.jkcq.hrwtv.service.daemon.util;
...
public class IntentWrapper {
    ...
    static {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            PowerManager pm = (PowerManager) BaseApp.getApp().getSystemService(Context.POWER_SERVICE);
            boolean ignoringBatteryOptimizations = pm.isIgnoringBatteryOptimizations(BaseApp.getApp().getPackageName());
            if (!ignoringBatteryOptimizations) {
                Intent dozeIntent = new Intent(Settings.ACTION_REQUEST_IGNORE_BATTERY_OPTIMIZATIONS);
                dozeIntent.setData(Uri.parse("package:" + BaseApp.getApp().getPackageName()));
        ...
        //三星 5.0/5.1 自启动应用程序管理
        Intent samsungLIntent = BaseApp.getApp().getPackageManager().getLaunchIntentForPackage("com.samsung.android.sm");
        ...
        meizuIntent.putExtra("packageName", BaseApp.getApp().getPackageName());
    ...
    public static String getApplicationName() {
        ...
        try {
            packageManager = BaseApp.getApp().getPackageManager();
            applicationInfo = packageManager.getApplicationInfo(BaseApp.getApp().getPackageName(), 0);
